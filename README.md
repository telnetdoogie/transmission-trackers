...
The following variables allow you to add trackers to **private torrents**. This is an advanced feature with some serious implications:

 **EXPLICIT WARNING - READ BEFORE USE:**

**Modifying trackers on private torrents can have serious consequences:**
- **Account Bans**: Private torrent sites may ban your account if trackers are modified on a private torrent.
- **Ratio Issues**: Tracker changes may corrupt your upload/download ratio tracking on private sites
- **TOS Issues**: Bypassing private tracker restrictions likely violates terms of service for that tracker
- **Loss of Access**: You may permanently lose access to private torrent communities

**This feature should ONLY be used if you fully understand and accept these risks.** It is designed for specific use cases like replacing dead/zombie trackers with alternatives.
It is not an alternative for cross-seeding across trackers; use [cross-seed](https://github.com/cross-seed/cross-seed) for that.

### Configuration

| Variable                       | Description                                                                                                       | Default Value |
|--------------------------------|-------------------------------------------------------------------------------------------------------------------|---------------|
| `OVERRIDE_PRIVATE`             | Set to `true` to enable private torrent tracker replacement. **Must be explicitly set to activate this feature.** | `False`       |
| `ZOMBIE_TRACKERS_LIST`  | Comma-separated list of zombie tracker domains (for example `"zombietracker.org, deadtorrent.com"`)               | `<empty>`      |


### How It Works

1. When enabled, if a private torrent is found, the app:
   * Checks if any of the torrent's current trackers (partial) match the list from `ZOMBIE_TRACKERS_LIST`
   * If a match is found, this torrent will get updated trackers as a normal, public torrent would

### **REMINDER**

- **This app assumes ZERO responsibility for bans, data loss, or other consequences from using this feature**
